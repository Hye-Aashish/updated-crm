...
export function useDashboardMetrics(settings: any) {
    ...
    const relevantProjects = useMemo(() => {
        ...
        if (currentUser?.role === 'client') {
            return projects.filter(p => p.clientId === currentUser.clientId || p.clientId === userId)
        }
    ...
    }, [canViewAll, projects, tasks, userId, currentUser])
    ...
    const relevantInvoices = useMemo(() => {
        if (canViewAll) return invoices
        if (currentUser?.role === 'client') {
            return invoices.filter(i => i.clientId === currentUser.clientId || i.clientId === userId)
        }
        const myProjectIds = relevantProjects.map(p => p.id)
        return invoices.filter(i => (i.projectId && myProjectIds.includes(i.projectId)) || i.clientId === userId)
    }, [canViewAll, invoices, relevantProjects, currentUser, userId])

    const relevantExpenses = useMemo(() => {
        if (canViewAll) return expenses
        const myProjectIds = relevantProjects.map(p => p.id)
        return expenses.filter(e => e.projectId && myProjectIds.includes(e.projectId))
    }, [canViewAll, expenses, relevantProjects])

    const totalRevenue = useMemo(() => relevantInvoices ? relevantInvoices.filter(i => i?.status === 'paid').reduce((sum, i) => sum + (i?.total || 0), 0) : 0, [relevantInvoices])
    const totalExpenses = useMemo(() => relevantExpenses ? relevantExpenses.reduce((sum, e) => sum + (e?.amount || 0), 0) : 0, [relevantExpenses])
    const netProfit = totalRevenue - totalExpenses
    const outstandingInvoices = useMemo(() => relevantInvoices ? relevantInvoices.filter(i => i?.status === 'pending' || i?.status === 'overdue').reduce((sum, i) => sum + (i?.total || 0), 0) : 0, [relevantInvoices])
    ...
    const revenueData = useMemo(() => {
        ...
        relevantInvoices.filter(i => i.status === 'paid').forEach(inv => {
        ...
        return last6Months
    }, [relevantInvoices, relevantExpenses])
    ...
    const recentActivities = useMemo(() => {
        ...
        // Latest Lead (Only for admins/owners)
        if (canViewAll) {
        ...
        // Latest Project
        const latestProject = [...relevantProjects].sort((a, b) => new Date(b.createdAt || 0).getTime() - new Date(a.createdAt || 0).getTime())[0]
        if (latestProject) {
            activityList.push({
        ...
        // Latest Task
        const latestTask = [...relevantTasks].sort((a, b) => new Date(b.createdAt || 0).getTime() - new Date(a.createdAt || 0).getTime())[0]
        if (latestTask) {
            activityList.push({
        ...
        // Latest Paid Invoice
        const latestPaid = [...relevantInvoices].filter(i => i.status === 'paid').sort((a, b) => new Date(b.updatedAt || 0).getTime() - new Date(a.updatedAt || 0).getTime())[0]
        if (latestPaid) {
            activityList.push({
    ...
    }, [leads, relevantProjects, relevantTasks, relevantInvoices, canViewAll])
    ...
    if (Array.isArray(roleLayout)) {
        ...
        if (userRole === 'client') {
            roleLayout = {
                enabledSections: ['hero', 'tasks', 'projects_overview', 'support_tickets', 'activity'],
                customLabels: {
                    projects_overview: 'My Projects',
                    tasks: 'My Tasks',
                    support_tickets: 'Support Tickets'
                },
                sectionOrder: ['hero', 'projects_overview', 'tasks', 'support_tickets', 'activity'],
                hiddenSubItems: ['mini_stats', 'velocity', 'conversion', 'revenue', 'expenses', 'profit', 'session']
            }
        } else if (userRole === 'pm') {
            roleLayout = {
                enabledSections: ['hero', 'session', 'deadlines', 'tasks', 'projects_overview', 'support_tickets', 'health', 'activity'],
                customLabels: {
                    projects_overview: 'Managed Projects',
                    tasks: 'Project Tasks'
                },
                sectionOrder: ['hero', 'session', 'projects_overview', 'tasks', 'deadlines', 'support_tickets', 'health', 'activity'],
                hiddenSubItems: ['revenue', 'expenses', 'profit', 'funnel', 'analytics', 'financials']
            }
        } else if (userRole === 'employee') {
            roleLayout = {
                enabledSections: ['hero', 'session', 'tasks', 'projects_overview', 'activity'],
                customLabels: {
                    projects_overview: 'My Projects',
    ...
    const relevantTickets = useMemo(() => {
        ...
        if (currentUser?.role === 'client') {
            return tickets.filter(t => (t as any).clientId === currentUser.clientId)
        }
        const myProjectIds = relevantProjects.map(p => p.id)
        return tickets.filter(t =>
            ((t as any).projectId && myProjectIds.includes((t as any).projectId)) ||
            t.assignedTo === userId ||
            t.assignedTo === currentUser.name
        )
    }, [canViewAll, tickets, relevantProjects, currentUser, userId])
    ...
    const resolvedTickets = relevantTickets.filter(t => t.status === 'resolved' || t.status === 'closed').length
    const criticalTickets = relevantTickets.filter(t => t.priority === 'critical' || t.priority === 'high').length
    ...
    const teamPerformance = useMemo(() => {
        ...
        return users.map(user => {
            ...
            const hourScore = Math.min(30, (hours / 40) * 30)
    ...
    }, [tasks, timeEntries, currentUser])

    const myPerformanceScore = useMemo(() => {
        const me = teamPerformance.find(p => (p as any)?.id === userId)
        return me ? (me as any).performanceScore : 0
